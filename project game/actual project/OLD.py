...
PNG_PATH = os.path.join(BASE_PATH, "MinesweeperPNGs")
...
class Minesweeper_grid(Grid):
    ...
    def draw_grid(self, png_size, topgrid_x, topgrid_y):
        png_dict = {
            x: os.path.join(PNG_PATH, f"Minesweeper_{x}.png") 
            for x in range(1, 9)
        }

        png_dict.update({
            f"GRASS{x}": os.path.join(PNG_PATH, f"Minesweeper_Grass{x}.png")
           for x in range(1, 3)
        })

        png_dict.update({
            0: os.path.join(PNG_PATH, "Minesweeper_EMPTY.png"),
            "BOMB": os.path.join(PNG_PATH, "Minesweeper_BOMB.png"),
            "FLAG": os.path.join(PNG_PATH, "Minesweeper_FLAG.png")
        })
            ...
            for col in range(self.width):
                ...
                if self.arr[row][col][0]:  # Revealed
                    if self.arr[row][col][1] == "BOMB":
                        ...
                        self.screen.blit(
                            png_dict[self.arr[row][col][1]],
                            rect.topleft
                        )

                        if self.arr[row][col][1] == 0:
                            for x, y in self.get_surrounding_sqrs(col, row):
                                self.arr[y][x][0] = True
                    ...
                    self.screen.blit(
                        png_dict["GRASS" + str(((row + col) % 2) + 1)],
                    rect.topleft)
